...
const Routing = () => {
  const route = (path: string, element: JSX.Element, children: RouteObject[] | undefined) => ({ path, element, children });

  const pages = [];
  // ホーム画面
  pages.push(route(`${routes.HOME}`, <Home />, undefined));
  // メンバーページ
  pages.push(route(`${routes.MEMBER}`, <Member />, undefined));
